...
num_particles <- 1e3 + 23# Number of particles used in the conditional BPF
...
deviate_states_init <- c(log(dirichlet_levels[1:DD])) + rep(c(1, 2), times = DD/2)
...
# True latent state process noise variance
  ...
  init_bet_xa <- lapply(init_bet_xa,
                        ...
                        y = deviate_par_rate)
  init_bet_xa <- rep(list(init_bet_xa), times = NN)
}
