...
if (init_at_true) {
  init_sig_sq_x <- true_sig_sq[, 1,  drop = FALSE]
  init_phi_x    <- true_phi[, 1, drop = FALSE]
...
# True latent state process noise variance
  init_sig_sq_x <- matrix(c(2, 2, 2, 2, 2, 2),
                           nrow = DD, ncol = 1)
  # init_sig_sq_x <- matrix(true_sig_sq[, 1, drop = TRUE],
                           # nrow = DD, ncol = 1)
  # True autoregressive parameter for states
  init_phi_x <- matrix(c(-0.9, 0.1, -0.9, -0.1, 0.1, -0.9),
                        nrow = DD,
                        ncol = 1)
  # init_phi_x <- matrix(true_phi[, 1, drop = FALSE],
  #                      nrow = DD,
  #                      ncol = 1)
...
if (!is.null(dataSim[[2]]$u)) {
  par_inits <- list(init_sig_sq = init_sig_sq_x,
                    init_phi = init_phi_x,
                    init_bet_z = init_bet_z,
                    init_bet_u = init_bet_u)
  prior_u <- true_D0u_u
} else {
  par_inits <- list(init_sig_sq_x = init_sig_sq_x,
                    init_phi_x = init_phi_x,
                    init_bet_z = init_bet_z)
  prior_u <- NULL
}
