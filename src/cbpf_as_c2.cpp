...
// [[Rcpp::export]]
arma::vec f_cpp2(const arma::vec& x_tt,
                 const double& phi_x,
                 const double& z_add) {
...
List cbpf_as_c2(const int& N,
  ...
  for (int t = 1; t < TT; ++t) {
    ...
    a.col(t) = id_as;
    // propagation
    eval_f = f_cpp2(xa1.col(t - 1), phi_xa1, Za1_beta1[t]);
    eval_f = eval_f.elem(id_as);
    xa1.col(t) = eval_f + sqrt(sig_sq_xa1)*arma::randn(N, 1);

    eval_f = f_cpp2(xa2.col(t - 1), phi_xa2, Za2_beta2[t]);
    eval_f = eval_f.elem(id_as);
    xa2.col(t) = eval_f + sqrt(sig_sq_xa2)*arma::randn(N, 1);

    eval_f = f_cpp2(xa3.col(t - 1), phi_xa3, Za3_beta3[t]);
    eval_f = eval_f.elem(id_as);
    xa3.col(t) = eval_f + sqrt(sig_sq_xa3)*arma::randn(N, 1);

    eval_f = f_cpp2(xa4.col(t - 1), phi_xa4, Za4_beta4[t]);
    eval_f = eval_f.elem(id_as);
    xa4.col(t) = eval_f + sqrt(sig_sq_xa4)*arma::randn(N, 1);

    eval_f = f_cpp2(xa5.col(t - 1), phi_xa5, Za5_beta5[t]);
    eval_f = eval_f.elem(id_as);
    xa5.col(t) = eval_f + sqrt(sig_sq_xa5)*arma::randn(N, 1);

    eval_f = f_cpp2(xa6.col(t - 1), phi_xa6, Za6_beta6[t]);
    eval_f = eval_f.elem(id_as);
