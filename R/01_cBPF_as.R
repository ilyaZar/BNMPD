cBPF_as <- function(y, yz, Za, Zb, Zp, Zq,
                    KK, N, TT,
                    sig_sq_xa1, phi_xa1, bet_xa1, xa1_r,
                    sig_sq_xa2, phi_xa2, bet_xa2, xa2_r,
                    sig_sq_xa3, phi_xa3, bet_xa3, xa3_r,
                    sig_sq_xa4, phi_xa4, bet_xa4, xa4_r,
                    filtering = TRUE) {
  if (!filtering) {
    xa1 <- matrix(rep(log(xa1_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
    xa2 <- matrix(rep(log(xa2_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
    xa3 <- matrix(rep(log(xa3_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
    xa4 <- matrix(rep(log(xa4_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
    w  <- matrix(1/N, nrow = N, ncol = TT)
    return(list(w, xa1, xa2, xa3, xa4))
  }
  # DATA CONTAINERS
  # particles for state processes:
  xa1 <- matrix(0, nrow = N, ncol = TT)
  xa2 <- matrix(0, nrow = N, ncol = TT)
  xa3 <- matrix(0, nrow = N, ncol = TT)
  xa4 <- matrix(0, nrow = N, ncol = TT)
  # xa1 <- matrix(rep(log(xa1_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
  # xa2 <- matrix(rep(log(xa2_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
  # xa3 <- matrix(rep(log(xa3_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
  # xa4 <- matrix(rep(log(xa4_t), times = N), nrow = N, ncol = TT, byrow = TRUE)
  # ancestors
  a  <- matrix(0, nrow = N, ncol = TT)
  # weights
  w  <- matrix(0, nrow = N, ncol = TT)
  # I. INITIALIZATION (t = 0)
  # Sampling initial condition from prior
  xa1[, 1] <- rnorm(n = N, mean = Za[1, , drop = F] %*% bet_xa1/(1 - phi_xa1),
                   sd = sqrt(sig_sq_xa1/(1 - phi_xa1^2)))
  xa2[, 1] <- rnorm(n = N, mean = Zb[1, , drop = F] %*% bet_xa2/(1 - phi_xa2),
                   sd = sqrt(sig_sq_xa2/(1 - phi_xa2^2)))
  xa3[, 1] <- rnorm(n = N, mean = Zp[1, , drop = F] %*% bet_xa3/(1 - phi_xa3),
                   sd = sqrt(sig_sq_xa3/(1 - phi_xa3^2)))
  xa4[, 1] <- rnorm(n = N, mean = Zq[1, , drop = F] %*% bet_xa4/(1 - phi_xa4),
                   sd = sqrt(sig_sq_xa4/(1 - phi_xa4^2)))
  # weighting (set to 1/N since there is no measurement y_t=0 at t=0)
  w[, 1]  <- 1/N
  # II. FIRST PERIOD APPROXIMATION (t = 1)
  # resampling
  a[, 1]  <- sample.int(n = N, replace = TRUE, prob = w[, 1])
  # propagation
  eval_fa <- f(x_tt = xa1[, 1], z = Za[1, , drop = F],
               phi_x = phi_xa1, bet_x = bet_xa1)
  xa1[, 1] <- eval_fa[a[, 1]] + sqrt(sig_sq_xa1)*rnorm(N)
  eval_fb <- f(x_tt = xa2[, 1], z = Zb[1, , drop = F],
               phi_x = phi_xa2, bet_x = bet_xa2)
  xa2[, 1] <- eval_fb[a[, 1]] + sqrt(sig_sq_xa2)*rnorm(N)
  eval_fp <- f(x_tt = xa3[, 1], z = Zp[1, , drop = F],
               phi_x = phi_xa3, bet_x = bet_xa3)
  xa3[, 1] <- eval_fp[a[, 1]] + sqrt(sig_sq_xa3)*rnorm(N)
  eval_fq <- f(x_tt = xa4[, 1], z = Zq[1, , drop = F],
               phi_x = phi_xa4, bet_x = bet_xa4)
  xa4[, 1] <- eval_fq[a[, 1]] + sqrt(sig_sq_xa4)*rnorm(N)
  # conditioning
  xa1[N, 1] <- xa1_r[1]
  xa2[N, 1] <- xa2_r[1]
  xa3[N, 1] <- xa3_r[1]
  xa4[N, 1] <- xa4_r[1]
  # weighting
  w_log   <- w_BPF(y = y[1, ], yz = yz[1, ], KK = KK,
                   xa1 = xa1[, 1],
                   xa2 = xa2[, 1],
                   xa3 = xa3[, 1],
                   xa4 = xa4[, 1])
  w_max   <- max(w_log)
  w_tilde <- exp(w_log - w_max)
  w[, 1]  <- w_tilde/sum(w_tilde)
  # resampling
  a[, 1]  <- sample.int(n = N, replace = TRUE, prob = w[, 1])
  # II. FOR t = 2,..,T
  for (t in 2:TT) {
    # resampling
    a[, t]     <- sample.int(n = N, replace = TRUE, prob = w[, t - 1])
    # propagation
    eval_fa    <- f(x_tt = xa1[, t - 1], z = Za[t, , drop = F],
                    phi_x = phi_xa1, bet_x = bet_xa1)
    xa1[, t]    <- eval_fa[a[, t]] + sqrt(sig_sq_xa1)*rnorm(N)
    eval_fb    <- f(x_tt = xa2[, t - 1], z = Zb[t, , drop = F],
                    phi_x = phi_xa2, bet_x = bet_xa2)
    xa2[, t]    <- eval_fb[a[, t]] + sqrt(sig_sq_xa2)*rnorm(N)
    eval_fp    <- f(x_tt = xa3[, t - 1], z = Zp[t, , drop = F],
                    phi_x = phi_xa3, bet_x = bet_xa3)
    xa3[, t]    <- eval_fp[a[, t]] + sqrt(sig_sq_xa3)*rnorm(N)
    eval_fq    <- f(x_tt = xa4[, t - 1], z = Zq[t, , drop = F],
                    phi_x = phi_xa4, bet_x = bet_xa4)
    xa4[, t]    <- eval_fq[a[, t]] + sqrt(sig_sq_xa4)*rnorm(N)
    # conditioning
    xa1[N, t]   <- xa1_r[t]
    xa2[N, t]   <- xa2_r[t]
    xa3[N, t]   <- xa3_r[t]
    xa4[N, t]   <- xa4_r[t]
    # ancestor sampling
    m1 <- matrix(c(eval_fa - xa1_r[t],
                   eval_fb - xa2_r[t],
                   eval_fp - xa3_r[t],
                   eval_fq - xa4_r[t]
                  ),
                nrow = N, ncol = 4)
    m2 <- diag(c(sig_sq_xa1^{-1},
                 sig_sq_xa2^{-1},
                 sig_sq_xa3^{-1},
                 sig_sq_xa4^{-1}
                )
              )
    m          <- -1/2 * helper_as(M = m2, x = m1)
    w_log_as   <- log(w[, t - 1]) + m
    w_max_as   <- max(w_log_as)
    w_tilde_as <- exp(w_log_as - w_max_as)
    w_as       <- w_tilde_as/sum(w_tilde_as)
    a[N, t]    <- sample.int(n = N, size = 1, replace = TRUE, prob = w_as)
    # weighting
    w_log   <- w_BPF(y = y[t, ], yz = yz[t, ], KK = KK,
                     xa1 = xa1[, t],
                     xa2 = xa2[, t],
                     xa3 = xa3[, t],
                     xa4 = xa4[, t])
    w_max   <- max(w_log)
    w_tilde <- exp(w_log - w_max)
    w[, t]  <- w_tilde/sum(w_tilde)
  }
  # trajectories
  ind <- a[, TT]
  for (t in (TT - 1):1) {
    xa1[, t] <- xa1[ind, t]
    xa2[, t] <- xa2[ind, t]
    xa3[, t] <- xa3[ind, t]
    xa4[, t] <- xa4[ind, t]
    ind     <- a[ind, t]
  }
  return(list(w, xa1, xa2, xa3, xa4))
}
